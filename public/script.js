...
function displayOnboardingCommands() {
  const commands = ["help", "projects", "resume", "theme light"];
  const row = document.createElement("div");
  row.className = "command-output command-chip-row";

  const label = document.createElement("span");
  label.className = "command-chip-label";
  ...
  commands.forEach((command) => {
    ...
    chip.setAttribute(
      "aria-label",
      `Insert command ${command} into terminal input`,
    );
  ...
  if (inputLine && inputLine.parentNode === cliOutput) {
    cliOutput.insertBefore(row, inputLine);
  } else {
