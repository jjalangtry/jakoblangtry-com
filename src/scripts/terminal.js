...
const virtualFileSystem = buildPortfolioFileSystem(terminalData);
...
function completeVirtualPath(arg, directoriesOnly = false) {
  const rawArg = String(arg || "");
  const slashIndex = rawArg.lastIndexOf("/");
  const baseArg = slashIndex >= 0 ? rawArg.slice(0, slashIndex + 1) : "";
  const partial = slashIndex >= 0 ? rawArg.slice(slashIndex + 1) : rawArg;
  const lookupBase = baseArg || ".";
  const entries = getVirtualDirectoryEntries(
    virtualFileSystem,
    currentDirectory,
    lookupBase,
  );
  ...
  return entries
    ...
    .filter((entry) =>
      entry.name.toLowerCase().startsWith(partial.toLowerCase()),
    )
    .map((entry) => {
      const suffix = entry.type === "directory" ? "/" : "";
      return `${baseArg}${entry.name}${suffix}`;
    });
}
...
└──────────────────────────────────────────────────────────────────────────┘
  ...
  ──────────────────────────────────────────────────────────────────────────`;
    ...
    case "ls":
      {
        const result = formatVirtualLsOutput(
          virtualFileSystem,
          currentDirectory,
        );
        appendOutput(result.output, "info-text");
      }
      break;
    case "clear":
...
Currently seeking opportunities in software engineering.`,
    ...
    case "alias":
      ...
      break;
    case "cat":
      appendOutput("Usage: cat [file]\nExample: cat about.txt", "info-text");
      break;
    case "tree":
      appendOutput(
        renderVirtualTree(virtualFileSystem, currentDirectory).output,
        "info-text",
      );
      break;
    case "skills":
    ...
    default:
      if (normalizedCommand === "converter") {
        ...
        const dir = normalizedCommand === "cd" ? "~" : command.substring(3);
        if (dir.trim() === "--help") {
          displayCommandHelp("cd");
          break;
        }
        const nextPath = resolveVirtualPath(currentDirectory, dir);
        const nextNode = getVirtualNode(virtualFileSystem, nextPath);
        if (!nextNode) {
          appendOutput(
            `cd: no such file or directory: ${dir.trim() || "~"}`,
            "error-text",
          );
        } else if (nextNode.type !== "directory") {
          appendOutput(`cd: not a directory: ${dir.trim()}`, "error-text");
        } else {
        ...
        if (arg === "--help") {
          ...
          appendOutput(
            ...
            result.error ? "error-text" : "info-text",
          );
        }
        break;
        ...
        if (arg === "--help") {
          displayCommandHelp("tree");
        } else {
          const result = renderVirtualTree(
            virtualFileSystem,
            currentDirectory,
            arg,
          );
...
function getHelpDetails() {
  return {
    ...
    cat: {
      ...
      notes:
        "Paths can be absolute, relative, or start with ~. Directories cannot be read with cat; use ls or tree instead.",
    },
    cd: {
      desc: "Change the current directory in the read-only portfolio filesystem.",
      usage: "cd [path]",
      examples: ["cd projects", "cd ..", "cd ~/blog", "cd /home/guest"],
      notes:
        "The prompt shows the current directory. Use cd with no path to return to /home/guest.",
    },
    countdown: {
    ...
    ls: {
      ...
      examples: ["ls", "ls projects", "ls ~/blog", "ls --commands"],
      notes:
        "Use ls --commands to list supported commands. Filesystem paths can be absolute, relative, or start with ~.",
    },
    man: {
    ...
    pwd: {
      desc: "Print the current directory in the portfolio filesystem.",
      usage: "pwd",
      examples: ["pwd"],
      notes:
        "The terminal starts in /home/guest and updates the prompt after cd.",
    },
    resume: {
    ...
    tree: {
      desc: "Render a directory tree for part of the portfolio filesystem.",
      usage: "tree [path]",
      examples: ["tree", "tree projects", "tree ~/blog"],
      notes:
        "Tree output is capped to a few levels so large repository and blog catalogs stay readable.",
    },
    uptime: {
  ...
  input.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Tab") {
      ...
      if (spaceIdx > 0) {
        ...
        const argLower = currentLower.substring(spaceIdx + 1);
        let completions = [];
        if (baseCmd === "blog") {
          completions = getAllPosts()
            .map((p) => p.slug)
            .filter((s) => s.startsWith(argLower));
        } else if (baseCmd === "delete") {
          completions = loadLocalPosts()
            .map((p) => p.slug)
            .filter((s) => s.startsWith(argLower));
        } else if (baseCmd === "theme") {
          completions = ["dark", "light"].filter((s) => s.startsWith(argLower));
        } else if (baseCmd === "man" || baseCmd === "help") {
          completions = commandList.filter((c) => c.startsWith(argLower));
        } else if (["cat", "cd", "ls", "tree"].includes(baseCmd)) {
          completions = completeVirtualPath(arg, baseCmd === "cd");
